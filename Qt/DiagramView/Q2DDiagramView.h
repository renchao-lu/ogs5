/**
 * \file Q2DDiagramView.h
 * KR Initial implementation
 */

#ifndef Q2DDIAGRAMVIEW_H
#define Q2DDIAGRAMVIEW_H

#include <QtGui/QWidget>
#include <QGraphicsView>
#include "Q2DDiagramScene.h"

/**
 * A simple view in which to display a Q2DDiagramScene. 
 * It supports resizing of the window and loading of data into the diagram.
 */
class Q2DDiagramView : public QGraphicsView
{
	Q_OBJECT
public:
	/**
	 * Creates an empty view.
	 */
	Q2DDiagramView(QWidget* parent = 0);
	/**
	 * Creates a view already containing a graph
	 * \param list Contains a list of data points and metainformation to be displayed by the scene.
	 */
	Q2DDiagramView(QDiagramList* list, QWidget* parent = 0);
	~Q2DDiagramView();

	/// Adds a new graph to the scene.
	void addGraph(QDiagramList* list);
	/// Returns the height of the bounding rectangle of all objects within the scene.
	int getHeight();
	/// Returns the width of the bounding rectangle of all objects within the scene.
	int getWidth();

protected:
	/// Resizes the scene.
	void resizeEvent(QResizeEvent* event);

private:
	void initialize();
	void keepItemAspectRatio();
        QSize minimumSizeHint() const;
        QSize sizeHint() const;
	void update();

	Q2DDiagramScene* _scene;
};

#endif //Q2DDIAGRAMVIEW_H
