...
void Gmsh2GeoIO::loadMeshAsGeometry (std::string & fname, GEOLIB::GEOObjects* geo)
{
	...
	for (size_t k(0); k<n_elements; k++) {
		...
		if (ele_type == 2) { // read 3 node triangle
			...
			pos_beg = pos_end+1;
			pos_end = line.find(" ", pos_beg);
			const size_t n_tags (str2number<size_t>(line.substr(pos_beg, pos_end-pos_beg)));
			// (over) read tags
			...
			pos_beg = pos_end+1;
			pos_end = line.find(" ", pos_beg);
			const size_t id0 (str2number<size_t>(line.substr(pos_beg, pos_end-pos_beg)) - 1); // shift -1!
			// parse second id of triangle
			pos_beg = pos_end+1;
			pos_end = line.find(" ", pos_beg);
			const size_t id1 (str2number<size_t>(line.substr(pos_beg, pos_end-pos_beg)) - 1); // shift -1!
			// parse third id of triangle
			pos_beg = pos_end+1;
			pos_end = line.find(" ", pos_beg);
			const size_t id2 (str2number<size_t>(line.substr(pos_beg, pos_end-pos_beg)) - 1); // shift -1!
