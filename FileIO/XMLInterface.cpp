...
void XMLInterface::readPoints( const QDomNode &pointsRoot,
...
void XMLInterface::readPolylines( const QDomNode &polylinesRoot,
...
void XMLInterface::readSurfaces( const QDomNode &surfacesRoot,
...
void XMLInterface::readStations( const QDomNode &stationsRoot,
                                  ...
                                  const QString &geoName)
{
	...
	for (int i = 0; i < lists.count(); i++)
	{
		if      (lists.at(i).nodeName().compare("BoundaryConditions") == 0)
			readConditions(lists.at(
			                       i), conditions, geoName,
			...
			readConditions(lists.at(i), conditions, geoName, FEMCondition::SOURCE_TERM);
	}
	if (!conditions.empty())
...
void XMLInterface::readConditions( const QDomNode &listRoot,
                                   std::vector<FEMCondition*> &conditions,
                                   const QString &geoName,
...
void XMLInterface::writeBoreholeData(QDomDocument &doc,
