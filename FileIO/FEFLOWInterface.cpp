...
CFEMesh* FEFLOWInterface::readFEFLOWModelFile(const std::string &filename)
{
  ...
  while(!feflow_file.eof()) 
  {
    ...
    else if(line_string.compare("DIMENS")==0) 
    {
      // DIMENS
      line_stream.str(GetLineFromFile1(&feflow_file));
      line_stream >> fem_dim.n_nodes >> fem_dim.n_elements >> fem_dim.n_nodes_of_element >> ws;
      line_stream.clear();
      // Create nodes
    ...
    {
      const long no_nodes_per_layer = (fem_class.dimension==2)? fem_dim.n_nodes : fem_dim.n_nodes / (fem_class.n_layers3d + 1);
          ...
          for(int j=0;j<12;j++) {
            ...
            if (fem_class.dimension==2) {
              ...
              for(int l=0;l<fem_class.n_layers3d+1;l++) {
                ...
                long nn = i*12 + no_nodes_per_layer_1 + j;
    ...
    {
      if (fem_class.dimension==2) continue;
  ...
  if (nodesEle.isNull()) return;
  {
    ...
    QString str_pt_list1 = xmlEle.text().simplified();
    {
      ...
      while (!ss.eof()) {
        ...
        double pt_xyz[3] = {};
        ss >> pt_id;
        for (int i=0; i<fem_class.dimension; i++) ss >> pt_xyz[i];
