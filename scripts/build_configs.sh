...
do
	...
	if [ "$config" = "PETSC_GEMS" ] ; then
		cmake_args="-VV -DPETSC_DIR=/opt/petsc-3.4.2 -DCMAKE_C_COMPILER=/opt/openmpi-1.4.1/bin/mpicc -DCMAKE_CXX_COMPILER=/opt/openmpi-1.4.1/bin/mpic++ -DMPI_INCLUDE_PATH=/opt/openmpi-1.4.1/include"
	fi
