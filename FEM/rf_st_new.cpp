...
//    dis_file_name.clear();
    ...
    for(size_t i=0; i<size; i++) delete normal2surface[i];
    size = pnt_parameter_vector.size();
    for(size_t i=0; i<size; i++) delete pnt_parameter_vector[i];
    //WW---------------------------------------
}
...
**************************************************************************/
ios::pos_type CSourceTerm::Read(std::ifstream *st_file, const GEOLIB::GEOObjects& geo_obj, const std::string& unique_name)
{
	char line[MAX_ZEILE];
		...
		const GEOLIB::GEOObjects& geo_obj, const std::string& unique_name)
{
	...
	if (geo_type_name.find("POINT") != string::npos) {
		...
		if (!((geo_obj.getPointVecObj(unique_name))->getPointIDByName (geo_name, _geo_obj_idx))) {
			std::cerr << "error in CSourceTerm::Read: point name \"" << geo_name << "\" not found!" << std::endl;
			exit (1);
		}
		in.clear();
	}
			...
			for (size_t i=0; i<no_source_terms; i++) {//Need to find the memory size limits for the anal. solution.
				if (st_vector[i]->analytical) {
					...
					number_of_terms = max(st_vector[i]->number_of_terms,
							static_cast<long>(number_of_terms));
				}
			}
			if (no_an_sol > 0) {
...
void CSourceTerm::Write(fstream* st_file)
{
  ...
  *st_file << "  ";
  if(geo_type_name.compare("LINE")==0){
    *st_file << geo_type_name << " " << geo_id << endl;
  }
  else
  *st_file << geo_type_name << " " << geo_name << endl;
  //--------------------------------------------------------------------
  // TIM_TYPE
...
void CSourceTermGroup::SetPNT(CRFProcess* m_pcs, CSourceTerm* m_st, const int ShiftInNodeVector)
{
	...
	m_nod_val->msh_node_number = m_msh->GetNODOnPNT((*pnt)[m_st->getGeoObjIdx()])
			+ ShiftInNodeVector;
...
//WW  int dimension = m_st->analytical_material_group;
  ...
  for (i = 0; i < size; i++){
     if (analytical_processes[i] == m_st->pcs_pv_name){
        if(m_st->geo_type_name.compare("POLYLINE")==0){
          if(m_st->getGeoName().compare (analytical_processes_polylines[i]) == 0)
              process_no = i;
        }
        if(m_st->geo_type_name.compare("DOMAIN")==0)
          process_no = i;
