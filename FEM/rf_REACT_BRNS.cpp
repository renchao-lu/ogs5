    ...
    // for this node, loop over all the chemical components and update the values
      for ( int k = 0 ; k < num_Comp ; k++, i_times_num_Comp_plus_k++ )
      {
          ...
          if (this_pcs)
          {
              ...
              cur_ts_Conc[i_times_num_Comp_plus_k] = this_pcs->GetNodeValue( i , this_pcs->GetNodeValueIndex( this_pcs->pcs_primary_function_name[0] )+ 1 );
...
{
    long i;
	if (myProblem->getEndTime() <= myProblem->getCurrentTime() ) {				// this is the last call to the chemical solver and
	  ...
	  //------------------------
          ...
          // Run BRNS;
          invokebrns( &(cur_ts_Conc_temp[i*num_Comp_temp]), &(pre_ts_Conc_temp[i*num_Comp_temp]), &(pre_ts_Conc_temp[i*num_Comp_temp]), &num_Comp_temp, &time_step, &(boundary_flag_temp[i*num_Comp_temp]),  &(rt_BRNS[i]), &pos_x, &pos_y, &pos_z, REACT_BRNS::m_porosity_Node + i, &waterSaturation, NULL );
...
#ifdef USE_MPI_BRNS
            invokebrns( &(cur_ts_Conc[i*num_Comp]), &(pre_ts_Conc[i*num_Comp]), &(pre_ts_Conc_buf[i*num_Comp]), &num_Comp, &time_step, &(boundary_flag[i*num_Comp]), &(rt_BRNS[i]), &pos_x, &pos_y, &pos_z, REACT_BRNS::m_porosity_Node + i, &waterSaturation, NULL );
#else
            invokebrns( &(cur_ts_Conc[i*num_Comp]), &(pre_ts_Conc[i*num_Comp]), &(pre_ts_Conc[i*num_Comp]), &num_Comp, &time_step, &(boundary_flag[i*num_Comp]), &(rt_BRNS[i]), &pos_x, &pos_y, &pos_z, REACT_BRNS::m_porosity_Node + i, &waterSaturation, NULL );
#endif
      }
#endif
#ifdef USE_MPI_BRNS
...
#ifdef USE_MPI_BRNS
	  if (myrank==1) {
		  cout << "Total time spent in BRNS Coupling and Solver: " << timeSpentInBrnsCoupling << "s, " << timeSpentInBrnsSolver << "s " << endl;
	  }
#else
	  cout << "Total time spent in BRNS Coupling and Solver: " << timeSpentInBrnsCoupling << "s, " << timeSpentInBrnsSolver << "s " << endl;
#endif
...
void REACT_BRNS::ConvPorosityNodeValue2Elem ( int i_timestep )
{
	...
	for ( i=0 ; i < nElems ; i++ )
	{
		...
		for ( j = 0 ; j < m_Elem->GetNodesNumber ( false ) ; j++ )
		{
			...
			m_porosity_Elem[i] += 1.0 / m_porosity_Node[idx_Node] / number_of_nodes; // this is for harmonic mean
		}
