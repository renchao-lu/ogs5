...
GEMIPM2K is the calculation kernel of GEMS-PSI (http://gems.web.psi.ch). GEMS-PSI executables for various platforms are freely availabe for download.
The the kernel GEMIPM2K source code is available on request.
GEMIPM2K is currently coupled in a non-iterave sequential way to groundwater flow and transport. The coupling to the Richards flow module is under development.
		...
		for ( ii = 0; ii < nNodes; ii++ )
		{
			...
			if ( ! ( m_NodeStatusCH[i] == OK_GEM_AIA || m_NodeStatusCH[i] == OK_GEM_SIA ) )
			{
				...
				if ( m_NodeStatusCH[i] == ERR_GEM_AIA )
				{
					...
					m_Node->na->GEM_write_dbr ( "dbr_for_crash_node_init.txt" );
					m_Node->na->GEM_print_ipm ( "ipm_for_crash_node_init.txt" );
#ifdef USE_MPI_GEMS
				...
				else if ( m_NodeStatusCH[i] == ERR_GEM_AIA )
				{
...
//        CompProperties *m_cp = NULL;
		...
		m_kin[ii].dc_counter=0; //this is the starting dc
			...
			m_kin[ii].dc_counter+=dCH->nDCinPH[j] ; //add the number of components to the counter..after test for phasename...as this counter gives the starting position of the dependent components
                }
...
//		cout << " rank, calc node, ii " << myrank << " " << in <<" " <<ii<< endl;
		...
		// Convert to concentration: should be done always...
                REACT_GEM::MassToConcentration ( in,1 /*new timestep*/ ); // concentrations are now based on the fluid-gas volumes...
        ...
        for ( ii = 0; ii < nNodes; ii++ ) {
                ...
                if ( m_boundary[in] ) { // do this only without calculation if  on a boundary!!!
                        ...
                        if ( ! ( m_NodeStatusCH[in] == OK_GEM_AIA || m_NodeStatusCH[in] == OK_GEM_SIA ) ) {
                                                ...
                                                repeated_fail +=1;
                                                if ( repeated_fail >10 ) {
						cout << "more than 10 nodes failed this timestep, check chemical system!" << endl;
#ifdef USE_MPI_GEMS
                                                        MPI_Finalize(); //make sure MPI exits
#endif
                                                        exit ( 1 );
                        ...
                        if ( aktueller_zeitschritt > 0 ) { // not for first time step
                                ...
                                if ( pressure <= 0.0    /*valcumm suction in groundwater is not so realistic*/
                                     || pressure > 1.0e+15  /*some very high pressure*/
                                        ...
                                        cout << " high pressure " << pressure << endl;        // then set it to 1.0 bar = 1.0e5 Pa;
                                        pressure = 1.0e+05;
                                }
                                break;
                        } else {
...
double REACT_GEM::GetDCValueSpecies_MT ( long node_Index, int timelevel, int iDc ) { // same functionality as GetComponentValue_MT
        string str;
...
short REACT_GEM::SetNodePorosityValue_MT ( long node_Index, int timelevel, double m_porosity ) {
        ...
        for ( i=0; i < ( int ) pcs_vector.size() ; i++ ) {
                ...
                if ( m_pcs->pcs_type_name.compare ( "MASS_TRANSPORT" ) == 0 ) {
                        ...
                        } else if ( str.compare ( "POROSITY" ) == 0 ) {
                                m_pcs->SetNodeValue ( node_Index,m_pcs->GetNodeValueIndex ( str ) +timelevel, m_porosity );
                        }
                }
        }
...
//	cout << " Porosity: Element "<< i << " " <<m_porosity_Elem[i] << endl;
                ...
                if ( m_porosity_Elem[i] >= max_possible_porosity ) m_porosity_Elem[i]=max_possible_porosity; // upper limit of porosity
                if ( m_porosity_Elem[i] <= min_possible_porosity ) m_porosity_Elem[i]=min_possible_porosity; //lower limit of porosity..
...
void REACT_GEM::MassToConcentration ( long in /*idx of node*/ ,int i_timestep ) {
	 ...
	 double skal_faktor;
        ...
        if ( ( fluid_volume <= 0.0 ) ) {
                cout <<"fluid volume negative or zero" << fluid_volume  << " node " << in <<endl;
#ifdef USE_MPI_GEMS
                MPI_Finalize(); //make sure MPI exits
#endif
                exit ( 1 );
...
// store volumes for reuse during concentration_to_mass
        ...
        case 2:  // liquid flow
                m_excess_water[in] = m_fluid_volume[in]- m_porosity[in];
                m_xDC[in*nDC + idx_water]*= m_porosity[in]/m_fluid_volume[in]; //mol amount of water in first phase
                m_xPA[0] *= m_porosity[in]/m_fluid_volume[in];  // this should give the same value as the above statement ....is used in concentration2mass and mass2concentration
...
//	if ( fabs ( m_excess_gas[in] ) >= 0.01 ) cout << "node "<< in <<" m_excess_gas " << m_excess_water[in] <<endl;
        ...
        if ( flag_transport_b ==0 ) {
        ...
        } else { // here we adjust the b vector
                /*		for ( j=0 ; j < nDC; j++ )
                		{
                			i = l*nDC + j;
                			// a better way to do would be for all mobile species...or?
                ...
                for ( j=0 ; j < nIC; j++ ) {
                        ...
                        if ( idx_hydrogen == j ) 	m_soluteB[i] = m_bPS[j] - ( 2.0*m_xDC[in*nDC + idx_water] ) ; //carrier for zero(first phase)  is normally water!
...
//			cout << "GEM: conc negativ for species " << j << " at node " << l << "value: " << m_xDC[i] ;
                                m_xDC[i] = fabs ( GetDCValueSpecies_MT ( l, 0, ( int ) j ) ); // make sure it is greater/eq zero and take value from last timestep ---> second argument is zero!

//			cout << " corrected to: " << m_xDC[i] << endl;
                        }
                        // cout << "conc2mass: l " << l << " j "<< j << " i " << i << " por " << m_porosity[l] << " m_xDC " <<  m_xDC[i] << endl;
                }
        } else { // transport of b species...we have to construct the new b vector!
                for ( j=0 ; j < nIC; j++ ) {
                        i = l*nIC + j;
                        // also check if xDC values is negative
                        if ( m_soluteB[i] < 0.0 ) {
                                m_soluteB[i] = fabs ( GetDCValueSpecies_MT ( l, 0, ( int ) j ) ); // make sure it is greater/eq zero and take value from last timestep ---> second argument is zero!
                        }
                        m_soluteB[i] *= water_volume ;
                        if ( idx_hydrogen == j ) 	m_soluteB[i] += ( 2.0*m_xDC[l*nDC + idx_water] ) ;   //carrier for zero(first phase)  is normally water!
...
// kg44 26.11.2008 read in parameters for kinetics and GEM
                if ( line_string.find ( "$KINETIC_GEM" ) !=string::npos ) { // subkeyword found
                        ...
                        if ( d_kin.kinetic_model >=1 && d_kin.kinetic_model<=3 ) {
...
//      12,13,14  double n_1, n_2,n_3; // exponents for acidic, neutral and base cases for species one
                        ...
                        if ( d_kin.surface_model == 1 || d_kin.surface_model == 2 ) {// surface model 1 and 2....only one parameter...
        ...
        for ( ii=0;ii<m_kin.size();ii++ ) {
                        ...
                        // kinetic_mocel==3 only precipitation (no dissolution)
                {
                        ...
                        for ( j=m_kin[ii].dc_counter;j<m_kin[ii].dc_counter+dCH->nDCinPH[k];j++ ) {
                                if ( ! ( dCH -> ccDC[j]  == '0' ) && ! ( dCH->ccDC[j]  == 'X' ) && ! ( dCH->ccDC[j]  == 'Y' ) && ! ( dCH->ccDC[j]  == 'Z' ) ) { // do not include surface complexation species!
//				omega_phase[k] += CalcSaturationIndex ( j, in,tempC,press ); // loop over all components of the phase
                                        omega_components[in*nDC+j] = m_Node->DC_a ( j ); // we need this later for solid solutions....
                                        omega_phase[in*nPH+k] += m_Node->DC_a ( j ); // loop over all components of the phase
...
//		 	cout << omega_phase[k] << " " <<  mol_phase[k] << endl; // debug
                        ...
                        sa=REACT_GEM::SurfaceAreaPh (ii); // value for surface area in m^2...(specific surface area multiplied with volume of the phase)
                        ...
                        dmdt[in*nPH+k]= -1.0 * sa * ( pow ( 10.0,m_kin[ii].kinetic_parameters[3] ) * rra + pow ( 10.0,m_kin[ii].kinetic_parameters[4] ) * rrn + pow ( 10.0,m_kin[ii].kinetic_parameters[5] ) * rrb ); // rate is scaled to the total amount available via the surface area
...
// cout << m_dul[in*nDC+j] << endl;
                                                ...
                                                m_dll[in*nDC+j]= m_xDC[in*nDC+j]; // give some freedom
//					m_dll[in*nDC+j]= m_dul[in*nDC+j];
                                                ...
                                                m_dul[in*nDC+j]= m_xDC[in*nDC+j]; // give some freedom
//					m_dul[in*nDC+j]= m_dll[in*nDC+j];
                                        ...
                                        if ( m_dll[in*nDC+j] < 0.0 ) m_dll[in*nDC+j]=0.0; // no negative masses allowed
                                        if ( m_dul[in*nDC+j] <= 0.0 ) m_dul[in*nDC+j]=1.0e-6; // no negative masses allowed..give some freedom
                                        if ( m_dll[in*nDC+j] > m_dul[in*nDC+j] ) m_dll[in*nDC+j]=m_dul[in*nDC+j];
...
double REACT_GEM::SurfaceAreaPh ( long kin_phasenr) {
        double surf_area=0.0;

        surf_area=m_Node->Ph_Volume ( m_kin[kin_phasenr].phase_number) ; // now it is volume of the phase ..with respect to the unit volume in m^3

        if ( m_kin[kin_phasenr].surface_model == 1 ) {
                surf_area *= m_kin[kin_phasenr].surface_area[0]; // multiplication with specific surface area gives area
        } else if ( m_kin[kin_phasenr].surface_model == 2 ) {
                surf_area= m_kin[kin_phasenr].surface_area[0];  // constant surface area
        } else {
...
int REACT_GEM::IsThisPointBCIfYesStoreValue ( long index, CRFProcess* m_pcs, double& value ) {
        for ( long p=0; p< m_pcs->bc_node_value.size(); ++p ) {
...
void REACT_GEM::WriteVTKGEMValues ( fstream &vtk_file ) {
        ...
        long i,j;
// this is point data
//    vtk_file << "POINT DATA " << nNodes << " " << endl;
