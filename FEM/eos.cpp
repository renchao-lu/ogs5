...
lamda_0 = 0.51826 * eta_0 * (3.75-f_int*(phi_id_tt+1.5)); //(Eq. 14)


P_sigma = P_c * exp(H[0]*T_star/(1-T_star)+H[1]*T_star+H[2]*pow(T_star,epsilon)+H[3]*pow(T_star,2)+H[4]*pow(T_star,3)); //(Eq. 3)
Z_c = P_c/(R*T_c*rho_c);
...
lamda_ex = 6.29638 * ( sum + j[6]*pow(delta,2)/delta_star); // (Eq. 17)
...
eta_0 = c1 * pow(c2*T,0.5) / (pow(sigma,2)*Omega); //eta in [Pa*s]
...
eta_0 = 1e6*(c1 * pow(c2*T,0.5) / (pow(sigma,2)*Omega)); //eta in [Pa*s]
...
switch (letter)
     ...
     case 'C' : // CARBON DIOXIDE
       ...
       // ideal gas part
		k[0][0]=  8.37304456;	k[0][1]=  -3.70454304;	k[0][2]=  2.5;			k[0][3]=  1.99427042; //a
		k[0][4]=  0.62105248;	k[0][5]=  0.41195293;	k[0][6]=  1.04028922;	k[0][7]=  0.08327678;
		k[1][0]=  0;		    k[1][1]=  0;		    k[1][2]=  0;			k[1][3]=  3.15163;	//theta
		k[1][4]=  6.1119;	    k[1][5]=  6.77708;	    k[1][6]=  11.32384;		k[1][7]=  27.08792;

		//real gas part

			K[0][0]=  3.8856823203161E-01; K[0][1]=  2.9385475942740E+00; K[0][2]= -5.5867188534934E+00; //n
			...
			K[1][0]=  1;	K[1][1]=  1;	K[1][2]=  1;	K[1][3]=  1;	K[1][4]=  2;	K[1][5]=  2; //d
			...
			K[2][0]= 0;		K[2][1]= 0.75;	K[2][2]= 1;		K[2][3]= 2;		K[2][4]= 0.75;	K[2][5]= 2; //t
			...
			K[3][7]= 1;		K[3][8]= 1;		K[3][9]= 1;		K[3][10]= 1;	K[3][11]= 1;	K[3][12]= 1; //c
			...
			K[4][39]= 3.5;		K[4][40]= 3.5;		K[4][41]= 3;  //a
			K[5][39]= 0.875;	K[5][40]= 0.925;	K[5][41]= 0.875; //b
			K[6][39]=0.7;		K[6][40]=0.7;		K[6][41]=0.7; //A
			K[7][39]=0.3;		K[7][40]=0.3;		K[7][41]=1;  //B
			...
			K[13][34]=1;	K[13][35]=1;		K[13][36]=1;		K[13][37]=1;	K[13][38]=1; // epsilon
