...
//				break;
		...
		} else if (m_out->dat_type_name.find("PVD") != string::npos) { 		// PVD (ParaView)
			if (m_out->vtk == NULL)
				m_out->vtk = new CVTK();
			CVTK* vtk = m_out->vtk;
...
void COutput::NODWriteDOMDataTEC()
{
  ...
  //----------------------------------------------------------------------
  // Tests
  if((_nod_value_vector.size()==0) && (mfp_value_vector.size()==0)) //OK4704
    return;
  //......................................................................
  // MSH
				...
				if (m_pcs != NULL) { //WW
					if (NodeIndex[k] > -1) {
						val_n = m_pcs->GetNodeValue(
								m_msh->nod_vector[j]->GetIndex(), NodeIndex[k]); //WW
						tec_file << val_n << " ";
						if (m_pcs->type == 1212 && _nod_value_vector[k].find(
								"SATURATION") != string::npos) //WW
							tec_file << 1. - val_n << " "; //WW
					}
				}
			}
			for (size_t k = 0; k < mfp_value_vector.size(); k++) //OK4704
			{
...
//	m_msh = GetMSH();
	...
	// PCON
	const size_t nPconName (_pcon_value_vector.size()); //MX
	for (size_t k = 0; k < nPconName; k++) {
		tec_file << ", " << _pcon_value_vector[k] << ""; //MX
	}
	tec_file << endl;
...
void COutput::ELEWriteDOMDataTEC()
{
  ...
  if(getProcessType () != INVALID_PROCESS) // PCS
    tec_file_name += "_" + convertProcessTypeToString (getProcessType()); // 09/2010 TF msh_type_name;
...
void COutput::WriteELEValuesTECHeader(fstream &tec_file)
{
	...
	for (size_t i = 0; i < _ele_value_vector.size(); i++) {
		if (_ele_value_vector[i].find("VELOCITY") == string::npos) //WW
...
//		std::cout << " found " << convertProcessTypeToString (pcs_vector[i]->getProcessType()) << " process" << std::endl;
	...
	if (number == 0 || number == 1) { //WW if(number==1)
		std::string project_title_string = "Profiles along polylines"; //project_title;
		...
		//....................................................................
		for (size_t k = 0; k < mfp_value_vector.size(); k++) { //OK4709
	...
	//......................................................................
	tec_file << " ZONE T=\"TIME=" << _time << "\"" << endl; // , I=" << NodeListLength << ", J=1, K=1, F=POINT" << endl;
	//----------------------------------------------------------------------
	// Write data
	//======================================================================
	...
	for (j = 0; j < (long) nodes_vector.size(); j++) {
		...
		gnode = nodes_vector[m_ply->getOrderedPoints()[j]]; //WW
		//------------------------------------------------------------------
		for (size_t k = 0; k < no_variables; k++) {
...
//	m_msh = GetMSH();
	...
	if (time_step_number == 0) { //WW  Old: if(time_step_number==1)
		std::string project_title_string = "Time curves in points"; //project_title;
		tec_file << " TITLE = \"" << project_title_string
				<< "\"" << endl;
		...
		for (size_t k = 0; k < mfp_value_vector.size(); k++) //OK411
		...
		if (geo_name.find("POINT") == std::string::npos)
			tec_file << " ZONE T=\"POINT="
					<< getGeoTypeAsString() << geo_name << "\"" << endl; //, I=" << anz_zeitschritte << ", J=1, K=1, F=POINT" << endl;
		else
...
void COutput::WriteRFO()
{
  ...
  if(!_new_file_opened) remove(rfo_file_name.c_str()); //WW
...
//			itemp = node_value_index_vector[i];
	...
	if (no_pcs) {
		tec_file << time_current << " ";
		for (size_t i = 0; i < _nod_value_vector.size(); i++) {
			m_pcs = PCSGet(MASS_TRANSPORT, _nod_value_vector[i]);//Specified currently for MASS_TRANSPORT only.
			node_value_index_vector[i] = m_pcs->GetNodeValueIndex(
					_nod_value_vector[i]) + 1;
	...
	if (getProcessType() != INVALID_PROCESS) {
		...
		for (size_t k = 0; k < nName; k++) {
			...
			for (size_t i = 0; i < pcs->GetPrimaryVNumber(); i++) {
				...
				if (_nod_value_vector[k].compare("COUPLING") == 0) // JOD
				{
					NodeIndex[k]++;
					break;
				}
...
#endif
  vtk_file_name += ".vtk";
  if(!_new_file_opened) remove(vtk_file_name.c_str()); //KG44
...
void COutput::CalcELEFluxes()
{
	...
	if (isDeformationProcess(pcs_type) || !isFlowProcess (pcs_type)
			|| pcs->m_msh->geo_name.find("REGIONAL") != string::npos) //WW
		return;
	//----------------------------------------------------------------------
	switch (getGeoType()) {
