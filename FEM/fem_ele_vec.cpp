...
//		} else if (pcs_vector[i]->pcs_type_name.find("PS_GLOBAL") != string::npos) {
		} else if (pcs_vector[i]->getProcessType() == PS_GLOBAL) { // TF
			h_pcs = pcs_vector[i];
			if (h_pcs->type == 1313)
...
//    	if (pcs_vector[i]->pcs_type_name.find("HEAT") != string::npos) {
		if (pcs_vector[i]->getProcessType() == HEAT_TRANSPORT) { // TF
			t_pcs = pcs_vector[i];
			break;
		}
	}
