...
int  M1CopyToAMG1R5Structure( double *A, int *IA, int *JA, int NDA, int NDIA, int NDJA, double *, double *, double *, double *);
...
int  M2CopyToAMG1R5Structure( double *A, int *IA, int *JA, int NDA, int NDIA, int NDJA, double *, double *, double *, double *);
...
#define Minim(x,v) if(x>(v))x=v        /* x =min(x,v)
                                          Modell 1: */
...
void *M34CreateMatrix(long param1, long param2, long param3)
{
  ...
  w -> PreD = (double *) Malloc(param1 * sizeof(double));       /* Diag. Precond. */
...
int M2Set(long i, long j, double aij)
{
  ...
  if(j<Zeil2(i).min_col) Zeil2(i).min_col = j;
  if(j>Zeil2(i).max_col) Zeil2(i).max_col = j;
...
double M2Get(long i, long j)
{
  ...
  if ((j<Zeil2(i).min_col)||(j>Zeil2(i).max_col)) return 0.;
...
int M2CopyToAMG1R5Structure(double *A, int *IA, int *JA, int NDA, int NDIA, int NDJA, double *x, double *U, double *b, double *F) {
    long i, j;
    long NNA = 0, NIA = 0;
    double a;
    Modell2 *w = (Modell2 *) wurzel;
...
int M34CopyToAMG1R5Structure(double *A, int *IA, int *JA, int NDA, int NDIA, int NDJA, double *x, double *U, double *b, double *F) {
    long i, j, NNA = 0, NIA = 0;
    double a;
    ...
    case 4:
      {
        ...
        for (k = 0; k < Sp34(ir).anz; k++)
          {                            /* ueber (links von) Diag. */
            ReSei[Ind34(ir, k)] -= Aik34(ir, k, 0) * Ri;        /* Obermatrix ausmultipl. */
            Aik34(ir, k, 0) = Aik34(ir, k, u) = 0.0;    /* Zeile / Spalte Null */
          }
                ...
                if (Ind34(i, k) == ir)
                  {
                    ...
                    Aik34(i, k, 0) = Aik34(i, k, u) = 0.0;      /* Zeile / Spalte Null */
                  }
              }
          }
    ...
    default:
      {
        ...
        rechts[index] = 0.;
        for (k = 0; k < anz_nachbarn; k++)
          MXSet(index, nachbarn_index[k], -gewicht * diag);     /* i ist die Zeile des irregulaere Knoten, nachbarn_index die Spalte */
                ...
                if (spalte == Ind2(zeile, k))
                  {                    /* gefunden! */
                    ...
                    for (l = 0; l < anz_nachbarn; l++)
                      MXInc(zeile, nachbarn_index[l], inc);     /* Den reg. Nachbarn zuschlagen */
                    break;
                  }
                else
                  {
              ...
              {                        /* Alle Eintraege durchsuchen */
                if (spalte == Ind2(zeile, k))
                  {                    /* gefunden! */
                    ...
                    Aik2(zeile, k) = 0.;        /* Eigenen Eintrag loeschen */
                    for (l = 0; l < anz_nachbarn; l++)
                      MXInc(nachbarn_index[l], spalte, inc);    /* Den reg. Nachbarn zuschlagen */
                    break;
                  }
                else
                  {
