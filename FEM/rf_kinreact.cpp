		...
		if (line_string.find("$PRODUCTIONTERMS") != string::npos) { // subkeyword found
			while (!new_subkeyword) {
				...
				if ((m_production->order != -99.0)
						&& (m_production->concentration != -1.0e9)) //check for read in
...
void CKinReact::ReadReactionEquation(string line_string_all) {
	...
	while (indexhigh < linelength) {
		...
		} else { //kein Leerzeichen gefunden, substring enth�lt nur den Namen => Zahl wird 1 gesetzt
			zahl = 1.0 * vz;
			stochmet.push_back(zahl);
...
void CKinReactData::Biodegradation(long node, double eps, double hmin,
	...
	double Csat_max, DensityNAPL, DensityAQ, DiffusionAQ, ViscosityAQ,
			PoreVelocity = 0.0, d50, Reynolds, Schmidt, Sherwood; //OK411
		...
		if (m_kr->typeflag_monod) {
			...
			if (BacteriaMass > 1.E-40) {
				...
				if (maxkap > 1.E-30) {
					...
					} else { /* Sterbeterm, grunds�tzlich keine Ber�cksichtigung des Kapazit�tsterms */
