	  ...
	  pcs(Pcs), dm_pcs(NULL), HEAD_Flag(false)
{
	...
	case 'P':                             // 04.03.2009 PCH
		...
		size_m = 40;
		break;
	case 'S':                             // PTCoupled-Flow //AKS/NB Mar2011
		for (i = 0; i < pcs->pcs_number_of_primary_nvals; i++)
			NodeShift[i] = i * pcs->m_msh->GetNodesNumber(false);
		idx0 = pcs->GetNodeValueIndex("PRESSURE1");
		idx1 = idx0 + 1;
	...
	if (pcs->Memory_Type == 0)            // Do not store local matrices
	{
		...
		if (pcsT == 'H' || pcsT == 'M' || pcsT == 'A' || pcsT == 'S')
		{
			Advection = new Matrix(size_m, size_m);
...
{
	int Size = nnodes;
	if(PcsType == V || PcsType == P || PcsType == S) //4.3.2009 PCH
		Size *= 2;
	...
	{
		if(PcsType == V || PcsType == P || PcsType == S) //04.3.2009 PCH
		...
		if(PcsType == H || PcsType == M || PcsType == A || PcsType == S)
		{
		...
		else if(pcs->pcs_type_number == 1)
		{
			...
			if(pcs->ML_Cap == 1)
			{
				...
				if (m_mmp->capillary_pressure_model_values[0] < MKleinsteZahl)
					...
					dPcdSw = 1.0 / m_mmp->SaturationPressureDependency(Sw,
					                                                   1000.0,
					                                                   1);
				mat_fac = time_unit_factor * k_rel /
...
void CFiniteElementStd::CalCoefLaplacePSGLOBAL(bool Gravity,  int dof_index)
{
	...
	case 2:
		...
		mfp_arg[0] = interpolate(NodalVal1) + MediaProp->CapillaryPressureFunction(0,
		                                                                           NULL,
		                                                                           0.0,
		                                                                           1,
		                                                                           Sw);
	...
	case 3:
		...
		if(pcs->num_type_name.find("dPcdSwGradSnw") != string::npos)
		{
			...
			// Pnw = Pw + Pc(Sw) // TODO: could cause errors in some cases
			mfp_arg[0] = interpolate(NodalVal1) + MediaProp->CapillaryPressureFunction(
			        0,
			        NULL,
			        0.0,
			        1,
...
double CFiniteElementStd::CalcSUPGCoefficient(double* vel,int ip)
{
	...
	if (PcsType == H)                     //heat
	{
		if (FluidProp->density_model == 14)
		{
			...
			dens_arg[2] = Index;
			diff /=
			        (FluidProp->SpecificHeatCapacity(dens_arg) *
...
/***************************************************************************
   ...
   02/2007   WW
 **************************************************************************/
void CFiniteElementStd::CalcMassPTC()
{
	int i, j,in,jn;
	// ---- Gauss integral
	int gp_r = 0,gp_s = 0,gp_t = 0;
...
void CFiniteElementStd::CalcLaplace()
{
	...
	int in_times_nnodes, jn_times_nnodes, i_plus_in_times_nnodes, j_plus_jn_times_nnodes,
	    dim_times_k, dim_times_k_plus_l, nnodes_plus_j, l_times_nnodes_plus_j,
	    k_times_nnodes_plus_i;
	...
	for (gp = 0; gp < nGaussPoints; gp++)
	{
		...
		for (in_times_nnodes = 0,in = 0; in < dof_n; in++, in_times_nnodes += nnodes)
			...
			for (jn_times_nnodes = 0,jn = 0; jn < dof_n;
			     jn++, jn_times_nnodes += nnodes)
			{
				...
				for (i = 0, i_plus_in_times_nnodes = in_times_nnodes; i < nnodes;
				     i++, i_plus_in_times_nnodes++)
				{
					//i_plus_in_times_nnodes = i + in_times_nnodes;

					for (j = 0,j_plus_jn_times_nnodes = jn_times_nnodes,
					     nnodes_plus_j = nnodes; j < nnodes;
					     j++, j_plus_jn_times_nnodes++, nnodes_plus_j++)
						for (k = 0,
						     dim_times_k = 0,k_times_nnodes_plus_i = i;
						     k < dim;
						     k++, dim_times_k += dim,
						     k_times_nnodes_plus_i += nnodes)
						{
							fkt_times_dshapefct__k_times_nnodes_plus_i__
							        = fkt *
							          dshapefct[k_times_nnodes_plus_i];
							for(l = 0,
							    dim_times_k_plus_l = dim_times_k,
							    l_times_nnodes_plus_j = j;
							    l < dim;
							    l++, dim_times_k_plus_l++,
							    l_times_nnodes_plus_j += nnodes)

								(*Laplace)(i_plus_in_times_nnodes,
								           j_plus_jn_times_nnodes)
								        +=
								                fkt_times_dshapefct__k_times_nnodes_plus_i__ \
								                *
								                mat[
...
/***************************************************************************
   ...
   01/2010   NW - SUPG
 **************************************************************************/
void CFiniteElementStd::CalcAdvectionPTC()
{
	int i, j, k, in, jn;
	int gp_r = 0, gp_s = 0, gp_t = 0;
	double fkt, mat_fac;
	...
	*AuxMatrix1   += *Laplace;
	...
	for (i = 0; i < nnodes; i++)
	{
		NodalVal0[i] =
		        fabs(pcs->GetNodeValue(nodes[i],
		                               idx) - x_n1[nodes[i] + NodeShift[pcs->continuum]]);
		NodalVal[i] = 0.0;
	}
	if(PcsType == V)                      //
	{
		...
		for (i = 0; i < nnodes; i++)
		{
			NodalVal0[i +
			          nnodes] =
			        fabs(pcs->GetNodeValue(nodes[i],
			                               idxp21) - x_n1[nodes[i] + NodeShift[1]]);
			NodalVal[i + nnodes] = 0.0;
		}
	}
	else if(PcsType == P)
	{
		...
		for (i = 0; i < nnodes; i++)
		{
			NodalVal0[i +
			          nnodes] =
			        fabs(pcs->GetNodeValue(nodes[i],
			                               idxSn1) - x_n1[nodes[i] + NodeShift[1]]);
			NodalVal[i + nnodes] = 0.0;
		}
	}
	//
	...
	for (i = 0; i < nnodes; i++)
	{
		NodalVal0[i] = atol + rtol* max(fabs(pcs->GetNodeValue(nodes[i],idx)),fabs(
		                                        x_n1[nodes[i] + NodeShift[pcs->continuum]]));
		NodalVal[i] = 0.0;
	}
	if(PcsType == V)                      //
