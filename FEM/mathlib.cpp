...
#endif
    vf[0] = (1.0 + r) * (1.0 + s) * (1.0 + t) \
        +alpha[0] * (1.0 + s) * (1.0 + t) + alpha[1] * (1.0 + r) * (1.0 + t) + alpha[2] * (1.0 + r) * (1.0 + s);
    vf[1] = (1.0 - r) * (1.0 + s) * (1.0 + t) \
        -alpha[0] * (1.0 + s) * (1.0 + t) + alpha[1] * (1.0 - r) * (1.0 + t) + alpha[2] * (1.0 - r) * (1.0 + s);
    vf[2] = (1.0 - r) * (1.0 - s) * (1.0 + t) \
        -alpha[0] * (1.0 - s) * (1.0 + t) - alpha[1] * (1.0 - r) * (1.0 + t) + alpha[2] * (1.0 - r) * (1.0 - s);
...
int MBistDuDiagMat(double *matrix, long m, long n)
{
    ...
    for (i = 0; i < m; i++)
        for (j = 0; j < n; j++)
            if ((fabs(matrix[i + j * n])>MKleinsteZahl) && ((i - j < -1) || (i - j > 1))) {  /*printf(" %ld %ld \n",i,j); */
                return 0;
            }
    diag = 3;
    knall = 0;
...
int MCompare_for_MQSort_LongDouble( const void *Arg1, const void *Arg2 )
{
  ...
  if (fabs(arg1->value-arg2->value)<MKleinsteZahl) return 0;
      else return -1;
}
