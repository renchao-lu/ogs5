...
//OK  DATCreateFileNames(dateiname);
  ...
  std::string tmp_fname (dateiname);
  tmp_fname += ".gli";
  std::cout << "GEOLIB reading " << tmp_fname << " ... " << std::flush;
  GEOLIB::GEOObjects *geo_obj (new GEOLIB::GEOObjects());
  FileIO::readGLIFileV4(tmp_fname, geo_obj);
  delete geo_obj;
