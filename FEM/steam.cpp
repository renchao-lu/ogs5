...

{
     ...
     double         b_vptx3, b_vptx3a, v, dv, pcalc=0.0, pct, vprev=0.0;
     static double  vptx3_save_0, vptx3_save_100;
     double         df, pprev=0.0, pctp=0.0, vup, vlow;
     int            vptx3_counter;
...
*/
           sprintf (str, \
              "Vptx3 - 100 iteration maximum limit exceeded (Call number %d)\n , \
               Actual pressure = %11.3f psia <> Calculated pressure = %11.3f psia\n , \
               Temperature = %11.3f F   <===>   Quality = %11.6f percent", \
              VPTX3_CALL, *pressure, pcalc, *temperature, *quality);
#ifdef WINDOWS
           MessageBox (NULL, str, SteamBoxTitle, MB_ICONSTOP & MB_ICONHAND);
#endif
...
{
     double    shvptd=0.0, shvptl=0.0, temp=0.0;
...
double tpshvx (double *cp, double *shv, double *quality,
...

{
     ...
     double    saturation_tp=0.0, dpdt;
     double    d_temp_press, tpshvx_shvcalc=0.0, tpshvx_percent=0.0, tpshvx_tp,
                   previous_tp=0.0, tp_up=0.0, tp_low=0.0, tp_df, tpshvx_prev_pct=0.0,
                   prev_tpshvx_shvcalc=0.0;
     int       tpshvx_counter;
     ...
     switch (option_tp) {
        case 1 :
           if (*cp >= PCA) saturation_tp = TC;
           else saturation_tp = tsl (cp);
           break;
        case 2 :
           if (*cp >= TC) saturation_tp = PCA;
           else saturation_tp = psl (cp, &dpdt);
        }
...
*/
           sprintf (str, \
                    "TPSHVX - 100 iteration maximum limit exceeded\n \
					 %s\n \
                     Steam Quality = %9.2f, Property = %12.6f ", \
                    str2, *quality, *shv);
#ifdef WINDOWS
           MessageBox (NULL, str, SteamBoxTitle, MB_ICONSTOP & MB_ICONHAND);
#endif
        ...
        switch (option_tp) {
           case 1 : tpshvx_shvcalc = shvptx (cp, &tpshvx_tp, quality,
                                             option_shv);
              break;
           case 2 : tpshvx_shvcalc = shvptx (&tpshvx_tp, cp, quality,
                                             option_shv);
           }
	if (MAJOR_ERROR) return 0.0;
        switch (option_tp) {
...

{
   ...
   //WW          if (c_quality != *quality) pk [i] = *saturation_pressure;
          if (fabs(c_quality -*quality)>MKleinsteZahl) pk [i] = *saturation_pressure;
...
/* OK Klammern */
     if (*pressure <= PCA || (*pressure > PCA && (pk [4] > psat &&
          ...
          if (pk [4] > psat && pk [6] <= psat) {
		   ...
		   //WW               if (pk [4] == psat || (pk [4] < psat && pk [2] > psat
               if (fabs(pk [4]- psat)<  MKleinsteZahl|| (pk [4] < psat && pk [2] > psat
                      && pk [3] > psat))
                  for (i = 0; i <= 4; i++) x = x + a [i] / v_critvel [i + 4];
     ...
     if (fabs(x )< MKleinsteZahl) return 0.0;
     else {
	gamma = pk [0] / x;
...
/* OK */
   ...
   if (fabs(*quality -100)< MKleinsteZahl)
