...
int main (int argc, char *argv[])
{
	...
	for (size_t k(0); k<n_ply; k++) {
		...
		if (ply_vec->getNameOfElementByID(k, ply_name)) {
			if (argc == 2)
				...
				if (ply_name.find (argv[2]) != std::string::npos) {
	...
	if (gli_out && bc_out) {
		...
		for (size_t k(0); k<ply_pnt_ids.size(); k++) {
			gli_out << k << " " << *((*geo_pnts)[ply_pnt_ids[k]]) << " $NAME PLYPNT" << argv[2] << k << std::endl;
			// boundary condition
			...
			bc_out << "\t$GEO_TYPE" << std::endl << "\t\tPOINT PLYPNT" << argv[2] << k << std::endl;
			bc_out << "\t$DIS_TYPE" << std::endl << "\t\tCONSTANT " << (*((*geo_pnts)[ply_pnt_ids[k]]))[2] << std::endl;
		}
