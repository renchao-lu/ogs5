...
void CFEMesh::computeMinEdgeLength()
{
	...
	if (s_edge_vec > 0) {
		...
		for (size_t k(1); k < s_edge_vec; k++) {
			...
			if (kth_edge_length < _min_edge_length) _min_edge_length
					= kth_edge_length;
		}
	}
}
		...
		if (line_string.find("#FEM_MSH") != std::string::npos) //12.08.2011. WW
		{
			...
			*fem_file >> no_nodes >> std::ws;
			...
			for (size_t i = 0; i < no_nodes; i++) {
				...
				*fem_file >> s;
				...
				} else fem_file->seekg(position, std::ios::beg);
				*fem_file >> std::ws;
			}
			...
			*fem_file >> no_elements >> std::ws;
			for (size_t i = 0; i < no_elements; i++) {
				...
				if (newElem->GetPatchIndex() > max_mmp_groups) max_mmp_groups
						= newElem->GetPatchIndex();
				//NW
				if (newElem->GetDimension() > this->max_ele_dim) this->max_ele_dim
...
void CFEMesh::ConstructGrid()
{
		...
		for (size_t i = 0; i < nFaces; i++) // Faces
		{
			...
			for (size_t k = 0; k < nFaceNodes; k++) //face nodes
			{
				...
				for (size_t ei = 0; ei < nConnElems; ei++) // elements connected to face node
				{
					size_t
							ee(
									static_cast<size_t> (e_nodes0[faceIndex_loc0[k]]->getConnectedElementIDs()[ei]));
					if (ee == e) continue;
					...
					for (size_t ii = 0; ii < nFacesConnElem; ii++) // faces of elements connected to face node
					{
						size_t
								nFaceNodesConnElem(
										static_cast<size_t> (connElem->GetElementFaceNodes(
												ii, faceIndex_loc)));
			...
			for (size_t i = 0; i < nFaces; i++) {
				...
				for (size_t k = 0; k < n0; k++) {
					...
					for (size_t ei = 0; ei < e_size_l; ei++) {
						size_t
								ee =
										e_nodes0[faceIndex_loc0[k]]->getConnectedElementIDs()[ei];
						CElem* connElem = ele_vector[ee];
		...
		for (size_t i = 0; i < nedges0; i++) // edges
		{
			...
			for (size_t k = 0; k < 2; k++) // beginning and end of edge
			{
				...
				for (size_t ei = 0; ei < nConnElem; ei++) // elements connected to edge node
				{
					size_t
							ee(
									e_nodes0[edgeIndex_loc0[k]]->getConnectedElementIDs()[ei]);
					if (ee == e) continue;
					CElem* connElem(ele_vector[ee]);
					const Math_Group::vec<long>& node_index_glb(
							connElem->GetNodeIndeces());
	...
	for (size_t e = 0; e < e_size; e++) {
		...
		if (elem->GetElementType() == MshElemType::LINE) continue; // line element
	...
	else if (xyz_dim[0] > 0.0 && xyz_dim[1] > 0.0 && xyz_dim[2] > 0.0) coordinate_system
			= 32;

	// 1D in 2D
	if (_msh_n_lines > 0) {
		if (xyz_dim[0] > 0.0 && xyz_dim[1] > 0.0 && xyz_dim[2] < MKleinsteZahl) coordinate_system
				= 32;
		if (xyz_dim[0] > 0.0 && xyz_dim[2] > 0.0 && xyz_dim[1] < MKleinsteZahl) coordinate_system
				= 32;
	}
	...
	for (size_t i = 0; i < ele_vector.size(); i++) {
		...
		if (elem->GetMark()) // Marked for use
		{
			if (coordinate_system == 21 || coordinate_system == 22) {
				if (elem->GetElementType() == MshElemType::LINE) elem->FillTransformMatrix();
			} else {
				if (elem->GetElementType() == MshElemType::LINE
						|| elem->GetElementType() == MshElemType::QUAD
						|| elem->GetElementType() == MshElemType::TRIANGLE) elem->FillTransformMatrix();
			}
		}
	}
}
...
long CFEMesh::GetNODOnPNT(const GEOLIB::Point* const pnt) const
{
	...
	ThreadParameter *thread_param0(new ThreadParameter(pnt, 0,
			static_cast<size_t> (nodes_in_usage / 4.0), nod_vector, 0));
	ThreadParameter *thread_param1(new ThreadParameter(pnt,
			static_cast<size_t> (nodes_in_usage / 4.0),
			static_cast<size_t> (nodes_in_usage / 2.0), nod_vector, 1));
	ThreadParameter *thread_param2(new ThreadParameter(pnt,
			static_cast<size_t> (nodes_in_usage / 2.0), static_cast<size_t> (3*
					nodes_in_usage / 4.0), nod_vector, 2));
	ThreadParameter *thread_param3(new ThreadParameter(pnt,
			static_cast<size_t> (3.0 * nodes_in_usage / 4.0), nodes_in_usage,
			nod_vector, 3));
	...
	if (thread_param0->_sqr_dist < thread_param1->_sqr_dist
			...
			&& thread_param0->_sqr_dist < thread_param3->_sqr_dist) return thread_param0->_number;
	if (thread_param1->_sqr_dist < thread_param0->_sqr_dist
			&& thread_param1->_sqr_dist < thread_param2->_sqr_dist
			&& thread_param1->_sqr_dist < thread_param3->_sqr_dist) return thread_param1->_number;
	if (thread_param2->_sqr_dist < thread_param0->_sqr_dist
			&& thread_param2->_sqr_dist < thread_param1->_sqr_dist
			&& thread_param2->_sqr_dist < thread_param3->_sqr_dist) return thread_param2->_number;
		...
		std::vector<size_t>& msh_nod_vector) const
{
	...
	for (size_t j(0); j < nodes_in_usage; j++) {
		if (sfc->isPntInBV((nod_vector[j])->getData(), _min_edge_length / 2.0)) {
			if (sfc->isPntInSfc((nod_vector[j])->getData())) msh_nod_vector.push_back(
		...
		std::vector<long>&msh_nod_vector) const
{
	...
	while (p_ply != m_sfc->polyline_of_surface_vector.end()) {
		...
		for (j = 0; j < NodesInUsage(); j++) {
			...
			if (fabs(Area1 - Area2) < Tol) msh_nod_vector.push_back(
					nod_vector[j]->GetIndex());
		}
		p_ply++;
	}
...
void CFEMesh::GetNODOnSFC_TIN(Surface*m_sfc, std::vector<long>&msh_nod_vector)
{
	...
	for (size_t i = 0; i < static_cast<size_t> (NodesInUsage()); i++) //NW cannot use nod_vector.size() because of higher order elements
	{
		...
		if ((pnt[0] >= sfc_min[0] && pnt[0] <= sfc_max[0]) && (pnt[1]
				>= sfc_min[1] && pnt[1] <= sfc_max[1]) && (pnt[2] >= sfc_min[2]
				&& pnt[2] <= sfc_max[2])) {
...
// TF								m_nod->SetCoordinates(xyz);
								m_nod->SetCoordinates(nod_vector[m_ele->nodes_index[i] + ((CountNLayers + 1) - row) * NNodesPerRow]->getData());
						...
						for (int i = NRowsToShift; i > 0; i--)
						{
							...
							else
							{
								for (j = 0; j < 3; j++)
								{
									...
									if (nod_vector[element_nodes[j]]->GetMark())
									{
										newz = nod_vector[element_nodes[j]]->getData()[2]
										+ (dz[j] * (iii + 1));
