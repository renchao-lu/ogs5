...
CGLPoint* GEOGetPointByName(string point_name)
{
  ...
  for(long i=0;i<(long)gli_points_vector.size();i++) {
    ...
    if(m_pnt->name.compare(point_name)==0) //OK
      return m_pnt; //OK
  }
#ifdef MFC
  AfxMessageBox("Warning: PNT not found");
#endif
  cout << "Warning: point " << point_name << " not found" << '\n';
  return NULL;
}
/**************************************************************************
...
**************************************************************************/
vector<CGLPoint*> GetPointsVector(void)
{
...
**************************************************************************/
vector<CGLPoint*> GEOLIB_SetGLIPoints_Vector(vector<CGLPoint*> gl_point)
{
...
**************************************************************************/
void GEOReadPoints(string file_name_path_base)
{
#ifdef MFC
...
ios::pos_type CGLPoint::Read(ifstream *gli_file,int* ok)
{
  ...
  if (!new_keyword) {
    ...
    if(line_string.find(hash)!=string::npos) {
     ...
     in.str(line); 
     int ok = sscanf(line,"%ld", &id);
		  if (ok==1)
		  {
            in >> id >> x >> y >> z;
            pos1 = 0;
            if(line_string.find("$MD")!=string::npos){
              pos1 = (int)line_string.find_first_of("M");
              sub_string = line_string.substr(pos1+2,string::npos);
              in.str(sub_string);
              in >> mesh_density;
             }
            if(line_string.find("$ID")!=string::npos){ //OK
              pos1 = (int)line_string.find_first_of("I");
              sub_string = line_string.substr(pos1+2,string::npos);
              in.str(sub_string);
              in >> name;
             }
...
**************************************************************************/
void GEOReadPointProperties(string file_name_base)
{
  CGLPoint *m_pnt = NULL;
  char line[MAX_ZEILEN];
  string sub_string;
  string line_string;
  //  ios::pos_type position;
  vector<string>pnt_properties_name_vector;
...
GEOLib-Method: 
Programing:
10/2005 CC Implementation
**************************************************************************/
long GEOPointID()
{
      CGLPoint *m_point = NULL;
      //char c_string_pnt[MAX_ZEILE];
//OK      CString m_Pointname;
      long size = 0;
...
CGLPoint* CGLPoint::Exist()
{
  ...
  for(int i=0;i<(int)gli_points_vector.size();++i){
    m_pnt_exist = gli_points_vector[i];
    if(PointDis(m_pnt_exist)<tolerance)
      return m_pnt_exist;
  }
  return NULL;
}
