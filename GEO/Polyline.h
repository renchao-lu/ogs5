...
public:
	 ...
	 */
	Polyline (const std::vector<Point*>& pnt_vec) : m_ply_pnts (pnt_vec) {};
	~Polyline() {};
	...
	size_t getSize () const { return m_ply_pnt_ids.size(); }

	/** returns true if the polyline is closed */
	bool isClosed () const {
		if (m_ply_pnt_ids.front() == m_ply_pnt_ids.back()) return true;
		else return false;
	}
