...
{
	CGridCell ThisCell(nCol, m_cellFocused.m_nRow);
	SetCellFocused( ThisCell);
}
...
//	GridCursorOffset tabOffsets[] = {goNext, goPrev};
	...
	if (gkArrows & m_nCursorKeys)  {
		switch (nChar)  {
		case VK_LEFT:
			aCell = GetCursorCell(m_cellFocused, goLeft);
			SetCursor(aCell, TRUE, TRUE);
			bSuccess = GetScrollInfo(SB_HORZ, &si);
			...
			break;
		case VK_RIGHT:
			aCell = GetCursorCell(m_cellFocused, goRight); 
			SetCursor(aCell, TRUE, TRUE);
			bSuccess = GetScrollInfo(SB_HORZ, &si);
			if (m_bRowSelect && bSuccess)  {
				si.fMask = SIF_POS;
		...
		case VK_DOWN:
			...
			//  Change selection
			aCell = GetCursorCell(cell, goDown);
			SetCursor(aCell, TRUE, TRUE);
			break;
		case VK_PRIOR:
			aCell = GetCursorCell(m_cellFocused, goPageUp);
			SetCursor(aCell, TRUE, TRUE);
			break;
		case VK_NEXT:
			aCell = GetCursorCell(m_cellFocused, goPageDown);
			SetCursor(aCell, TRUE, TRUE);
			break;
		case VK_HOME:
		...
		case VK_TAB:
			if (gkTabs & m_nCursorKeys)  {
				aCell = GetCursorCell(m_cellFocused, goNext);
				SetCursor(aCell, TRUE, TRUE);	//  NYI
			}
			break;
		default:
			break;
...
CGridHeaderSection * CGridHeaderSections::Add()
{
	...
	nIndex = (int)m_sections.Add(new CGridHeaderSection(this));
...
BOOL CGridHeaderSections::FindResizeSection(CPoint point, CGridHeaderSection *& pRetrievedSection)
{
	...
	for (i = (int)m_sections.GetUpperBound(); i >= 0; i --)  {
...
CGridColumn& CGridColumn::operator =(const CGridColumn& sourceColumn)
{
	...
	m_pickList.RemoveAll();
	nRetVal = (int)m_pickList.Append(sourceColumn.m_pickList);
...
void CGridColumn::GetPickList(CStringArray& pickList)
{
	...
	nRetVal = (int)pickList.Append(m_pickList);
}
...
void CGridColumn::SetPickList(const CStringArray &pickList)
{
	...
	m_pickList.RemoveAll();
	nRetVal = (int)m_pickList.Append(pickList);
}
...
CGridColumns::CGridColumns(const CGridColumns& otherColumns)
{
	...
	for (i = (int)m_columns.GetUpperBound(); i >= 0; i --)
		delete m_columns[i];
...
//		m_pGrid->ColumnsChange(this);
	...
	for (i = 0; i < otherColumns.m_columns.GetSize(); i ++)  {
		...
		nRetVal = (int)m_columns.Add(pColumn);
	}
...
CGridColumn * CGridColumns::Add()
{
	...
	if (m_pGrid == NULL)  {
		nIndex = (int)m_columns.Add(new CGridColumn(this));
		return m_columns.GetAt(nIndex);
	}
