// OGLMath.cpp special Vector-Management for 3D-Moves inside OpenGL
...
int oglvector::counter=0;// global counter initialization
...
//!the oglvector representing the origin
const oglvector ORIGIN(0,0,0);
//!the unit oglvector associated to the x axys
const oglvector X_AXIS(1,0,0);
//!the unit oglvector associated to the y axys
const oglvector Y_AXIS(0,1,0);
//!the unit oglvector associated to the z axys
const oglvector Z_AXIS(0,0,1);
...
//-----------------------------------------------------------------------------
// oglvector friends and members

oglvector operator+(const oglvector& v1,const oglvector& v2)
{
	return oglvector(v1.vec[0]+v2.vec[0], v1.vec[1]+v2.vec[1], v1.vec[2]+v2.vec[2]);
}
...
oglvector operator-(const oglvector& v1)
{
	return oglvector(-v1.vec[0],-v1.vec[1],-v1.vec[2]);
}

oglvector operator^(const oglvector& v1,const oglvector& v2)
{
	return oglvector( v1.vec[1]*v2.vec[2]-v1.vec[2]*v2.vec[1],
				  -v1.vec[0]*v2.vec[2]+v1.vec[2]*v2.vec[0],
				   v1.vec[0]*v2.vec[1]-v1.vec[1]*v2.vec[0]);
}

real operator*(const oglvector& v1,const oglvector& v2)
{
	return v1.vec[0]*v2.vec[0] + v1.vec[1]*v2.vec[1] + v1.vec[2]*v2.vec[2];
}

/*!
This function together with operator*(real,oglvector) implements the commutative product of a scalar by a oglvector
*/
oglvector operator*(const oglvector& v,const real& fact)
{
	return oglvector(fact*v.vec[0],fact*v.vec[1],fact*v.vec[2]);
}

/*!
This function together with operator*(oglvector,real) implements the commutative product of a scalar by a oglvector
*/
oglvector operator*(const real& fact,const oglvector& v)
{
	return oglvector(fact*v.vec[0],fact*v.vec[1],fact*v.vec[2]);
}
...
oglvector operator/(const oglvector& v,const real& fact)
{
	...
	return oglvector(v.vec[0]/fact,v.vec[1]/fact,v.vec[2]/fact);
}
	oglvector Bisect(oglvector v0,oglvector v1);//!< returns the unit oglvector which halves the arc between v0 and v1

oglvector Bisect(oglvector v0,oglvector v1)
{
	//add the vectors
	oglvector v=v0+v1;
	// normalize the sum oglvector or fill it with a standar oglvector
	real norm=v.length2();
...
#ifdef _DEBUG
CDumpContext& operator<<(CDumpContext& cd,const oglvector& vect)
{
...
/*!
This function modifies the oglvector upon which has been called.
\return the length of the oglvector before normalization, this is useful to check if the normalization operation has been numerically precise.
*/
real oglvector::normalize()
{
	real len=length();
...
/*!
This function doesn't modifies the oglvector upon which has been called, it returns a new oglvector instead.
\return the normalized copy of the oglvector
*/
oglvector oglvector::normalized() const
{
	real len=length();
	assert(fabs(len)>=epsilon);
	return oglvector(this->vec[0]/len,this->vec[1]/len,this->vec[2]/len);
}

void oglvector::EpsilonCorrect(const oglvector& v)
{
...
/*
operator oglvector::operator char*()
{
}
*/
...
quaternion operator*(const quaternion& q1,const quaternion& q2)
{
	...
	oglvector tv=q1.s*q2.v+q2.s*q1.v+(q1.v^q2.v);
...
/*!
\b NOTE: the norm is comparable to the squared length of a oglvector not to the length
*/
...
/*!
the rotation represented by the unitquaternion would transform the first oglvector
into the second. The vectors are of unit length (so they are placed on a unit sphere)
\param vfrom the first oglvector
\param vto the second oglvector
*/
void unitquaternion::getVectorsOnSphere(oglvector& vfrom,oglvector& vto)
{
	unitquaternion tmp=(*this)*(*this);
	real s=sqrt(tmp.x()*tmp.x()+tmp.y()*tmp.y());
	if(s<=epsilon) vfrom=Y_AXIS;
	else vfrom=oglvector(-tmp.y()/s,tmp.x()/s,0.0);
...
{
	// "quick & dirty" approach accessing the internal matrix as a oglvector
	real* rp=(real*)mat;
	for (int pos=0;pos<16;pos++)
		rp[pos]=val;
	counter++;
